...
class Trainer:
    def __init__(self, dataset, args, device):
        self.device = device
        self.model = SimplE(dataset.num_ent(), 
                            dataset.num_rel(), 
                            args.emb_dim, 
                            self.device)
    ...
    def train(self):
        ...
        for epoch in range(self.args.ne):
            ...
            while not last_batch:
                h, r, t, l = self.dataset.next_batch(
                ...
                # TODO make this a class... way to many values 
                scores,_,_,_,_,_,_ = self.model(h, r, t)
                loss = torch.sum(F.softplus(-l * scores))+ (self.args.reg_lambda * self.model.l2_loss()\
            ...
            # TODO return print statments when training takes more than 5 seconds...
