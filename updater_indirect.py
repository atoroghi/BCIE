...
# The main updating function that performs gaussian or laplace updating
	def compute_laplace_approximation(self):
		...
		if self.update_type == "laplace":
			...
			self.Z = np.zeros(self.emb_dim)
			z_map = self.SDR_cvxopt(self.tau_z, self.X, self.y , self.Z)
			self.Z = z_map
			X_new = np.multiply(self.likes_embedding, z_map)
			y_new = np.ones(np.shape(X_new)[0])
			self.X = X_new
			self.y = y_new
