...
class Recommender:
    ...
    def pre_critiquing_new(self):
      # user score for all movies 
      prod = np.multiply(self.user_embedding_head_proj, self.items_embeddings_tail)
      prod_inv = np.multiply(self.user_embedding_tail_proj, self.items_embeddings_head)
      scores = np.clip(np.sum(prod + prod_inv, axis=1), -40, 40)

      # UPDATE: multi-line for clarity
      scores1 = np.clip(np.sum(np.multiply(self.user_embedding_head_proj, self.items_embeddings_tail) + np.multiply(self.user_embedding_tail_proj ,self.items_embeddings_head),axis=1),-40,40)
